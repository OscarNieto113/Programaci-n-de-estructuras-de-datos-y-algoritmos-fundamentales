...
void List<T>::insert_at(T val, uint index) {
	...
	//Verificar si la posicion del elemento dada por el usuario es valida
	if (index > size){ //O(1)
		throw IndexOutOfBounds(); //O(1)
	}
	...
	//Ciclo para recorrer hasta la posicion indicada por el usuario
	while (p!=NULL){ //O(N)
		if (index==0){ //O(1)
			q = new Node <T>(val); //O(1)
			q ->next = head; //O(1)
			head = q; //O(1)
			size++; //O(1)
			std::cout << toString(); //O(1)
			break; //O(1)
		}

		if (aux + 1 >= index){ //O(1)
			q = new Node<T>(val); 				//1. Creamos el valor dado por el usuario en un nodo
			q->next = p->next;						//2. Le decimos que val es el siguiente de p
			p->next = q;									//3. p es el siguiente del nuevo valor
			size++;												// Aumentamos el tamanio de la lista
			std::cout << toString();
			break;
		}
