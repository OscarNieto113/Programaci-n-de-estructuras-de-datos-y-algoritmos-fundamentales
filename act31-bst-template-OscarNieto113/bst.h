...
uint Node<T>::leaves() const {
	...
	le= 0;								//O(1)
	ri=0;									//O(1)
	if (left == NULL && right == NULL){//O(1)
		return 1;						//O(1)
	}

	else{
		if (left != NULL){	//O(1)
			le = left->leaves();//Log2(n)
		}

		if (right !=NULL){
			ri = right->leaves();//Log2(n)
		}
	}
	acum = le + ri;				//O(1)
	return acum;					//O(1)
...
bool Node<T>::isFull() const {
	...
	else if (left != NULL && right != NULL){		//O(1)
		return left->isFull() && right->isFull();	//Log2(N)
	}
	else {
		return false;															//O(1)
	}
...
T Node<T>::ancestor(T val) const {
	if (val < value){										//O(1)
		if (left != NULL){								//O(1)
			if (left->value == val){				//O(1)
				return value;									//O(1)
			}
			else{
				return (left->ancestor(val));	//Log2(n)
			}
		}
		else {
		...
		if (right != NULL){								//O(1)
			if (right->value == val){				//O(1)
				return value;									//O(1)
			}
			else{
				return (right->ancestor(val));//Log2(n)
			}
		}
		else {
			throw NoSuchElement();
...
std::string BST<T>::byLevel() const {
	...
	std::queue< Node<T> *> queue;				//O(1)
	queue.push(root);										//O(1)
	aux << "[";													//O(1)
	if (!empty()) {											//O(1)
		while (!queue.empty()){						//O(n)
			Node <T> *temp = queue.front();	//O(1)
			queue.pop();										//O(1)
			aux<<temp ->value<< " ";				//O(1)
			if (temp->left != NULL){				//O(1)
				queue.push(temp->left);				//O(1)
			}
			if (temp->right != NULL){				//O(1)
				queue.push(temp->right);			//O(1)
			}
		}
	}
	aux << "]";													//O(1)
