...
template <class Vertex>
int VertexNotReached(const Vertex& start, const UnweightedGraph<Vertex>* graph, int MNP) {
	std::set<Vertex> visited;
	std::queue<Vertex> pending;
	typename std::set<Vertex>::iterator itr;

	std::set<Vertex> level;
	int x=0;

	pending.push(start); //Aniade el vertice que esta al inicio al queue no visitado
	while (!pending.empty()) { //Mientras haya vertices sin visitar
		Vertex v = pending.front(); //Al elemento que esta hasta arriba del queue conviertelo en vertice
		pending.pop(); //Saca al dicho elemento del queue
		if (visited.find(v) == visited.end()) { // Si el vertice no ha sido visitado
			visited.insert(v); // Marcalo como visitado
			std::set<Vertex> connected = graph->getConnectionFrom(v); //Obten todas las conexiones del vertice

			for (itr = connected.begin(); itr != connected.end(); itr++) { //Agrega todas las conexiones a pending
				pending.push( (*itr) );
				x++;
			}
		}
	}
	std::cout << x << " " << visited.size() << "\n";
	return (x);
}

template <class Vertex>
void printLevels(const Vertex& start, const UnweightedGraph<Vertex>* graph, int size){
	std::set<Vertex> visited;
	std::queue<Vertex> pending;
	typename std::set<Vertex>::iterator itr;
